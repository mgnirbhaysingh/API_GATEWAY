...
def extract_products(response_data: Dict) -> List[Product]:
    ...
    for snippet in snippets:
        # Check if this is the "Showing related products" container
        data = snippet.get('data', {})
        identity = data.get('identity', {})
        title = data.get('title', {})
        
        # Stop processing if we encounter the related products header
        if (identity.get('id') == 'product_container' and 
            title.get('text') == 'Showing related products'):
            print("Found 'Showing related products' section - stopping product extraction")
            break
...
cookie_temp = {}
def generate_similar_ip(base_ip="146.196.36.41", variation=10):
    parts = base_ip.split('.')
    third = max(0, min(255, int(parts[2]) + random.randint(-variation, variation)))
    fourth = max(1, min(254, int(parts[3]) + random.randint(-variation, variation)))
    return f"{parts[0]}.{parts[1]}.{third}.{fourth}"
...
def fetch_blinkit_data(query: str = None, lat: str = "28.4511202", lon: str = "77.0965147", next_url: str = None) -> Dict[str, Any]:
    ...
    headers = {
        ...
        "Lat": lat,
        "Lon": lon,
        "X-Forwarded-For": generate_similar_ip(),
    }
