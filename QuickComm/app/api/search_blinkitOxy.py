...
# Oxylabs Proxy Configuration (replace with your credentials)
PROXY_BASE_USER = "customer-mindcase_9B7XA"
PROXY_PASS = ""
PROXY_HOST = ""
PROXY_PORT = 7777
...
def get_proxies():
    ...
    proxy_user = f"{PROXY_BASE_USER}-sessid-{session_id}"
    proxy_url = f"http://{proxy_user}:{PROXY_PASS}@{PROXY_HOST}:{PROXY_PORT}"
    return {
        "http": proxy_url,
        "https": proxy_url
    }
# Removed PROXY_AUTH and do not use 'auth' in requests
...
@router.get("/blinkit/searchOxy")
def search_blinkit_oxy(query: str = "chocolate", coordinates: str = "28.451,77.096", save_to_db: bool = False) -> List[Dict[str, Any]]:
    lat, lon = coordinates.split(',')
    all_products = []
    page_count = 0
    has_next_url = True
